#!/usr/bin/env python3
"""Train LM with OrderKD + optional retention KD, using response-only SFT loss.

This is a variant of train_lm_orderkd.py where the Vietnamese LM loss is computed
only on response tokens (prompt tokens are masked with -100).

The Vietnamese training file is expected to contain a response marker (default:
"### Response:") in the `text` column.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train LM with orderKD + retention, response-only SFT loss")
...
class OrderKDCollator:
    ...
    def __call__(self, examples: List[dict]) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "dep_positions": torch.tensor(dep_positions_t, dtype=torch.long),
            "head_positions": torch.tensor(head_positions_t, dtype=torch.long),
...
class ResponseOnlyTextLMCollator:
    def __init__(self, tokenizer, max_length: int, text_column: str, response_marker: str, add_eos: bool) -> None:
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.text_column = text_column
        self.response_marker = response_marker
        self.add_eos = add_eos
        self.dropped_no_marker = 0
        self.dropped_empty_response = 0
        self.forced_label_token = 0

    def _split(self, text: str) -> tuple[str, str]:
        idx = text.find(self.response_marker)
        if idx < 0:
            return "", ""
        prompt = text[: idx + len(self.response_marker)]
        response = text[idx + len(self.response_marker) :]
    ...
    def __call__(self, examples: List[dict]) -> Dict[str, torch.Tensor]:
        ...
        for ex in examples:
            text = normalize_text_value(ex.get(self.text_column, ""))
            prompt_text, response_text = self._split(text)
            if not prompt_text or not response_text:
                if not prompt_text:
                    self.dropped_no_marker += 1
                else:
                    self.dropped_empty_response += 1
                continue
            ...
            if len(total_ids) > self.max_length:
                if len(response_ids) + len(eos) >= self.max_length:
                    total_ids = (response_ids + eos)[-self.max_length :]
                    prompt_len = 0
                else:
                    keep_prompt = self.max_length - (len(response_ids) + len(eos))
            ...
            input_ids_list.append(total_ids)
            labels_list.append(labels)
            attn_list.append([1] * len(total_ids))

        if not input_ids_list:
            # Avoid empty batches. Force 1 supervised token so loss is finite.
            tok = int(self.tokenizer.eos_token_id or 0)
            input_ids_list = [[tok]]
            labels_list = [[tok]]
            attn_list = [[1]]
            self.forced_label_token += 1
        ...
        # If a batch somehow ended up with all labels masked, force a single label token.
        if (labels_t != -100).sum().item() == 0:
            labels_t[0, 0] = int(batch["input_ids"][0, 0].item())
            self.forced_label_token += 1
...
def masked_kl(
    student_logits: torch.Tensor, teacher_logits: torch.Tensor, attention_mask: torch.Tensor, temperature: float
) -> torch.Tensor:
...
def main() -> None:
    ...
    lm_collator = ResponseOnlyTextLMCollator(
        tokenizer=tokenizer,
    ...
    # Retention KD dataset and frozen teacher model.
    ret_loader = None
    ret_iter = None
    base_model = None
    if args.lambda_ret > 0.0:
        base_name = args.base_model_name_or_path or args.model_name_or_path
        base_model = AutoModelForCausalLM.from_pretrained(base_name, dtype=dtype, cache_dir=args.cache_dir).to(device)
        base_model.eval()
