#!/usr/bin/env python3
"""Convenience launcher for VI Wikipedia continual pretraining with 8-bit QLoRA-style adapters.

This wraps `finetune_vi_baseline.py` and supplies the same defaults as the LoRA launcher,
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Launch 8-bit QLoRA-style continual pretraining on the VI Wikipedia corpus."
    )
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "outputs" / "checkpoints" / "viwiki_cpt_qlora8"),
    )
...
def main() -> None:
    ...
    print(f"[continual_pretrain_vi_qlora8] train={args.train_file}")
    if args.eval_file:
        print(f"[continual_pretrain_vi_qlora8] eval={args.eval_file}")
    if args.en_buffer_file:
        print(f"[continual_pretrain_vi_qlora8] en_buffer={args.en_buffer_file}")
    print(f"[continual_pretrain_vi_qlora8] output={args.output_dir}")
