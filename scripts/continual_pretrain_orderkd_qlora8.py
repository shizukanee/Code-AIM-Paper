#!/usr/bin/env python3
"""Convenience launcher for VI Wikipedia OrderKD continual pretraining with 8-bit QLoRA-style adapters.

This wraps `train_lm_orderkd.py` and supplies the same defaults as the LoRA launcher,
but forces 8-bit base loading plus LoRA adapter training.
"""
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Launch OrderKD + 8-bit QLoRA-style continual pretraining on the VI Wikipedia corpus."
    )
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "outputs" / "checkpoints" / "viwiki_cpt_orderkd_qlora8"),
    )
...
def main() -> None:
    ...
    print(f"[continual_pretrain_orderkd_qlora8] train={args.train_file}")
    print(f"[continual_pretrain_orderkd_qlora8] order_kd={args.order_kd_jsonl}")
    if args.en_buffer_file:
        print(f"[continual_pretrain_orderkd_qlora8] en_buffer={args.en_buffer_file}")
    print(f"[continual_pretrain_orderkd_qlora8] output={args.output_dir}")
